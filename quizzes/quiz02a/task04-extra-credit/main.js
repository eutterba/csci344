...
const doesTermMatch = (track) => {
  ...
  if (track.album.name.toLowerCase().includes(searchTerm.toLowerCase())) {
    match = true;
  }
  if (track.artist.name.toLowerCase().includes(searchTerm.toLowerCase())) {
    match = true;
  }
...
function showMatching() {
  const container = document.querySelector("#track-lists");
  ...
  let matches = tracks.filter(doesTermMatch);

  matches.forEach((track) => {
    const snippet = dataToHTML(track);
